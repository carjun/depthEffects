...
public:
	...
	static void imSIFT(const Image<T1> imsrc, Image<T2>& imsift, int cellSize = 2, int stepSize = 1,double normalization=0.05,bool IsBoundaryIncluded = false,int nBins = 8);

	template <class T1,class T2>
	static void imSIFT(const Image<T1> imsrc, Image<T2>& imsift, const vector<int> cellSizeVect, double normalization=0.05,int stepSize = 1,bool IsBoundaryIncluded = false, int nBins = 8);
    
    template <class T1,class T2>
    static void imPatch(const Image<T1>& imsrc,Image<T2>& impatch,int winSize=4);
};

template <class T1,class T2>
void ImageFeature::imPatch(const Image<T1>& imsrc,Image<T2>& impatch,int winSize)
{
    int width = imsrc.width(), height=imsrc.height(),nchannels=imsrc.nchannels();
    int winLen = winSize*2+1;
    int patchDim = nchannels*winLen*winLen;
    if(!impatch.matchDimension(width,height,patchDim))
        impatch.allocate(width,height,patchDim);
    
    for(int i = 0;i<height;i++)
        for(int j = 0;j<width;j++)
        {
            int offset = (i*width+j)*patchDim;
            for(int ii = -winSize; ii<=winSize; ii++)
                for(int jj = -winSize; jj<=winSize; jj++)
                {
                    int x = __min(__max(j+jj,0),width-1);
                    int y = __min(__max(i+ii,0),height-1);
                    int offset1 = ((ii+winSize)*winLen+jj+winSize)*nchannels;
                    int offset2 = (y*width+x)*nchannels;
                    for(int k = 0;k<nchannels;k++)
                        impatch.data()[offset+offset1+k] = imsrc.data()[offset2+k];
                }
        }
}

template <class T1,class T2>
void ImageFeature::imSIFT(const Image<T1> imsrc, Image<T2> &imsift,int cellSize,int stepSize,double normalization,bool IsBoundaryIncluded,int nBins)
{
	if(cellSize<=0)
	{
	...
	for(int i=0;i<sift_height;i++)
		for(int j =0;j<sift_width;j++)
		{
			...
			for(int k = 0;k<siftdim;k++)
				imsift.pData[offset+k] = (unsigned char)__min(sift_cell.pData[k]/(mag+normalization)*CONSTANT,255);//(unsigned char) __min(sift_cell.pData[k]/mag*512,255);
		}//*/
}
...
//--------------------------------------------------------------------------------------------------
template <class T1,class T2>
void ImageFeature::imSIFT(const Image<T1> imsrc, Image<T2> &imsift,const vector<int> cellSizeVect,double normalization,int stepSize,bool IsBoundaryIncluded,int nBins)
{
	// this parameter controls decay of the gradient energy falls into a bin
	// run SIFT_weightFunc.m to see why alpha = 9 is the best value
	int alpha = 9; 
