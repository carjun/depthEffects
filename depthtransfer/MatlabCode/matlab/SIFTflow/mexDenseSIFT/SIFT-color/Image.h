...
public:
    ...
    template <class T1,class T2>
    void Merge(const Image<T1>& image1,const Image<T2>& image2,double alpha=1, double beta=1);
...
template <class T>
template <class T1,class T2>
void Image<T>::Merge(const Image<T1>& image1,const Image<T2>& image2,double alpha=1, double beta=1)
{
    ...
    clear();
    allocate(image1.imWidth,image1.imHeight,image1.nChannels + image2.nChannels);
    for(int i = 0;i<imHeight;i++)
        for(int j = 0;j<imWidth;j++)
        {
            int offset = (i*imWidth+j)*nChannels;
            int offset1 = (i*imWidth+j)*image1.nChannels;
            int offset2 = (i*imWidth+j)*image2.nChannels;
            // get information from image 1
            if(alpha == 1)
                for(int k = 0;k<image1.nChannels;k++)
                    pData[offset+k] = image1.pData[offset1+k];
            else
                for(int k = 0; k<image1.nChannels;k++)
                    pData[offset+k] = image1.pData[offset1+k]*alpha;
            // get information from image 2
            if(beta == 1)
                for(int  k = 0; k < image2.nChannels; k++)
                    pData[offset+image1.nChannels+k] = image2.pData[offset2+k];
            else
                for(int k = 0; k < image2.nChannels; k++)
                    pData[offset+image1.nChannels+k] = image2.pData[offset2+k]*beta;
        }
}
